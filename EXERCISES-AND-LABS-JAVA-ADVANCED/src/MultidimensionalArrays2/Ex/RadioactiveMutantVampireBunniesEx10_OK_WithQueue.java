...
import java.util.Scanner;
    ...
    public class RadioactiveMutantVampireBunniesEx10_OK_WithQueue {
        ...
        public static void main(String[] args) {
            ...
            ArrayDeque<Integer> PositionsOfBQueue = new ArrayDeque<>();
            for (int row = 0; row < matrix.length; row++) {
                String[] input = scan.nextLine().split("");
                for (int col = 0; col < matrix[row].length; col++) {
                    matrix[row][col] = input[col].charAt(0);
                    if (matrix[row][col] == 'B') {
                        PositionsOfBQueue.offer(row);
                        PositionsOfBQueue.offer(col);
                    }
                    if (matrix[row][col] == 'P') {
            ...
            while (++counter < commands.length && !isPlayerDead) {
                ...
                moveBunnies(matrix, PositionsOfBQueue);
