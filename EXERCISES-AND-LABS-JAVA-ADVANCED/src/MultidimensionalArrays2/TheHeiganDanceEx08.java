...
public class TheHeiganDanceEx08 {
    ...
    private static boolean isInDamageArea(int targetRow, int targetCol, int rowAfterMoving, int colAfterMoving) {
        return ((targetRow - 1 <= rowAfterMoving && rowAfterMoving <= targetRow + 1)
                && (targetCol - 1 <= colAfterMoving && colAfterMoving <= targetCol + 1));
    }
}